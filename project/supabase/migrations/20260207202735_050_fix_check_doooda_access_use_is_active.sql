/*
  # Fix check_doooda_access to use is_active instead of active_provider_id

  1. Changes
    - Remove active_provider_id dependency from doooda_config
    - Check for is_active = true directly in ai_providers table
    - Simplify provider existence check to single query
    - Ensure function uses the new single source of truth (is_active field)

  2. Purpose
    - Establishes ai_providers.is_active as the ONLY source of truth
    - Removes confusing dual-provider configuration
    - Aligns with database triggers that enforce single active provider
