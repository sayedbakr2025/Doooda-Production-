...
BEGIN
  ...
  -- Match @[Full Name] format (supports spaces in names)
  FOR v_word IN 
  ...
  -- Also match @Word format (no spaces, backward compatible)
  FOR v_word IN 
    SELECT regexp_matches(p_content, '@(\S+)', 'g')
  LOOP
    v_mention_name := trim(v_word);
    -- Skip if this was already caught by @[...] pattern above
    IF POSITION(v_mention_name IN p_content) > 0 AND p_content !~ ('@\[' || v_mention_name) THEN
      PERFORM public._create_single_mention_notification(
        p_comment_id, v_mention_name,
        p_project_id, p_scene_id, p_author_id, p_comment_type,
        v_project_title, v_scene_title, v_author_name, v_cta_link
...
-- Helper function to create a single mention notification (avoids duplication)
